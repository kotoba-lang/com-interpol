(ns interpol.main
  "Kotodama WASM entrypoint for the Interpol clean-room actor (L4) — Clojure port.
  ...
  facts (`interpol.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "interpol")
(def tier "L4")
...
(def entity-specs
  [{:entity "Case"      :plural "cases"      :id-prefix "interpol_cas"
    :fields [:number :jurisdiction :status :filedAt] :required [:number :jurisdiction]
    :coerce {}                       :refs {}}
   {:entity "Party"     :plural "parties"    :id-prefix "interpol_par"
    :fields [:caseId :role :name]           :required [:role :name]
    :coerce {}                       :refs {:caseId "Case"}}
   {:entity "Document"  :plural "documents" :id-prefix "interpol_doc"
    :fields [:caseId :type :contentRef]     :required [:type]
    :coerce {}                       :refs {:caseId "Case"}}
   {:entity "Incident"  :plural "incidents" :id-prefix "interpol_inc"
    :fields [:type :jurisdiction :occurredAt] :required [:type :jurisdiction]
    :coerce {}                       :refs {}}
   {:entity "Watchlist" :plural "watchlists" :id-prefix "interpol_wat"
    :fields [:name :program :entries]       :required [:name :program]
    :coerce {:entries :int}          :refs {}}
   {:entity "Sanction"  :plural "sanctions" :id-prefix "interpol_san"
...
(defn emit-facts
  "EAVT facts for one record: {\"interpol.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "interpol-compat" :tier tier :entities entities} 200])
...
(defn start! [] :interpol-compat/ready)
